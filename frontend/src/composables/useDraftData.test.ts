  ...
  async function loadPicks(picks: Record<string, unknown>[]) {
    vi.mocked(fetch).mockImplementation(
      async (url) =>
        ({
          ok: true,
          json: async () => (String(url).includes('forfeited_picks.json') ? [] : picks),
        }) as unknown as Response,
    )
    const draftData = useDraftData()
    await draftData.loadDraftData()
...
describe('useDraftData forfeited picks', () => {
  ...
  it('shows forfeited picks alongside players by default', async () => {
    const draftData = await load(regularPicks, [
      { year: 2025, round: 1, pick: null, team: 'NYK', reason: 'Tampering' },
    ])
    expect(draftData.forfeitedFilter.value).toBe('show')
  ...
  it('hides forfeited picks in "hide" mode', async () => {
    const draftData = await load(regularPicks, [
      { year: 2025, round: 1, pick: null, team: 'NYK', reason: 'Tampering' },
    ])
    draftData.forfeitedFilter.value = 'hide'
